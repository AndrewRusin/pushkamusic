"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReleasesController = void 0;
const common_1 = require("@nestjs/common");
const releases_model_1 = require("./releases.model/releases.model");
const releases_service_1 = require("./releases.service");
const create_release_dto_1 = require("./dto/create-release.dto");
const releases_constants_1 = require("./releases.constants");
const jwt_guard_1 = require("../auth/guards/jwt.guard");
let ReleasesController = exports.ReleasesController = class ReleasesController {
    constructor(releasesService) {
        this.releasesService = releasesService;
    }
    async create(dto) {
        return this.releasesService.create(dto);
    }
    async allItems() {
        return this.releasesService.findAllItems();
    }
    async get(id) {
        const release = await this.releasesService.findById(id);
        if (!release) {
            throw new common_1.NotFoundException(releases_constants_1.RELEASE_NOT_FOUND_ERROR);
        }
        return release;
    }
    async delete(id) {
        const deletedRelease = await this.releasesService.deleteById(id);
        if (!deletedRelease) {
            throw new common_1.NotFoundException(releases_constants_1.RELEASE_NOT_FOUND_ERROR);
        }
    }
    async patch(id, dto) {
        const updateRelease = await this.releasesService.updateById(id, dto);
        if (!updateRelease) {
            throw new common_1.NotFoundException(releases_constants_1.RELEASE_NOT_FOUND_ERROR);
        }
        return updateRelease;
    }
};
__decorate([
    (0, common_1.UseGuards)(jwt_guard_1.JwtAuthGuard),
    (0, common_1.UsePipes)(new common_1.ValidationPipe()),
    (0, common_1.Post)('create'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_release_dto_1.CreateReleaseDto]),
    __metadata("design:returntype", Promise)
], ReleasesController.prototype, "create", null);
__decorate([
    (0, common_1.UsePipes)(new common_1.ValidationPipe()),
    (0, common_1.Get)('all'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], ReleasesController.prototype, "allItems", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], ReleasesController.prototype, "get", null);
__decorate([
    (0, common_1.UseGuards)(jwt_guard_1.JwtAuthGuard),
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], ReleasesController.prototype, "delete", null);
__decorate([
    (0, common_1.UseGuards)(jwt_guard_1.JwtAuthGuard),
    (0, common_1.UsePipes)(new common_1.ValidationPipe()),
    (0, common_1.Patch)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, releases_model_1.ReleasesModel]),
    __metadata("design:returntype", Promise)
], ReleasesController.prototype, "patch", null);
exports.ReleasesController = ReleasesController = __decorate([
    (0, common_1.Controller)('releases'),
    __metadata("design:paramtypes", [releases_service_1.ReleasesService])
], ReleasesController);
//# sourceMappingURL=releases.controller.js.map